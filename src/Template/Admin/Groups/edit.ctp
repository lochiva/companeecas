...
<div class="row">
    ...
    <div class="edit-user">
        ...
        <a class="btn btn-danger delete-user" href="<?=Router::url('/admin/groups/delete/' . $group->id)?>">Elimina</a>
...
<script>
    $(document).ready(function(){
        ...
        $("#user-caledar-view").select2();

        $('#tbl-users-groups').tablesorter({
            widthFixed: true,
        ...
        $('.delete-group-user').click(function(e){

            if(!confirm("Si è sicuri di voler eliminare rimuovere l'utente? L'operazione non sarà reversibile.")){
                e.preventDefault();
            }
...
<div class="row">
    <hr>
    <div>
        <h3><i class="glyphicon glyphicon-user"></i> Gestione Utenti del Gruppo</h3>
        <h5>Da questa sezione è possibile gestire gli utenti del gruppo.</h5>
    </div>
    <hr>
    <div>
        ...
        <div class="box-action">
          <div class="row">
            <?= $this->Form->create(null, ['url' => ['controller' => 'Groups', 'action' => 'addUserToGoup']]); ?>
            <div class="col-sm-10">
              <select class="form-control select2"  id="user-caledar-view" name="id_user">
            ...
            <?= $this->Form->input('id_group', ['type' => 'hidden', 'value' => $group->id, 'required'=>true]); ?>
            <div class="col-sm-2">
              <button class="btn btn-primary" style="float:right;" title="Aggiungi utente"><i class="glyphicon glyphicon-plus"></i></a>
            </div>
            <?= $this->Form->end() ?>
          </div>
        </div>
        <div style="clear: both"></div>
        <table id="tbl-users-groups" class="table table-striped table-hover ">
            <thead>
                <tr>
                    <th>Username</th>
                    <th>Nome</th>
            ...
            <tbody>
                <?php foreach ($group['users'] as $key => $user) { ?>
                    <tr>
                        ...
                        <td><?=$user->email ?></td>
                        <td>
                            <a class="btn btn-xs btn-danger delete-group-user" href="<?=Router::url('/admin/groups/deleteUserToGoup/' . $user->_joinData->id)?>" title="Elimina"><i class="glyphicon glyphicon-remove"></i></a>
                        </td>
                    </tr>
                <?php } ?>
            </tbody>
