  ...
  $("#table-report").trigger("update");
    ...
    function setTable(){
        ...
        if($("#datepicker").val() != '' && $("#datepicker2").val() != '' && $("#datepicker").val().length == 10 && $("#datepicker2").val().length == 10 ){
          ...
          $('#reportTitle').html('<i  class="fa fa-th"></i> REPORT ANALITICO ATTIVITA\' DAL ' + startDate + ' AL '+ endDate +' '+ lista_studi + (yearID == -1 ? '' : ' RIF '+$('#yearID option:selected').text()) );

        }else{

          $('#reportTitle').html('<i  class="fa fa-th"></i> REPORT ANALITICO ATTIVITA\' ' + (yearID == -1 ? '' : $('#yearID option:selected').text())+ ' ' + lista_studi);
        }
        ...
        $("#table-report").tablesorter({
              ...
              headers: { 4: { filter: false },5: { filter: false },6: { filter: false},7: { filter: false} }
          }).tablesorterPager({
                ...
                container: $(".pager"),
                ajaxUrl : pathServer + 'consulenza/Ws/getReportTotaleTasks/'+yearID+'/'+officeID+'/?{filterList:filter}&{sortList:column}&size={size}&page={page}&startDate='+startDate+'&endDate='+endDate+' ',
    ...
    $(document).ready(function(){
      ...
      $('#escludiAnno').change(function(){
        setTable();
        setFilter();
      });

      $('#xls_export').click(function(){
          window.open(pathServer + 'consulenza/report/totaleTasks/'+($('#escludiAnno').prop('checked')? '-1' : $('#yearID option:selected').text())+'/'+($('#officeID').val() != null ? $('#officeID').val() : -1)+'/xls','_self');
      });
...
<section class="content-header">
  <h1>
    Report Analitico attività:
    <small>Report di tutte le attività con relative ore e clienti</small>
  </h1>
  <input type="hidden" id="clienteID" value="" />
  <ol class="breadcrumb">
    <li class="active">Report Analitico attività</li>
  </ol>
</section>
                ...
                <div class="box-body">
                	<div class="row">
                		<div class="col-lg-6 col-md-12">
                          ...
                          <?php
                            $date = '';
                            if($this->request->session()->read('Report.TotaleTasks.startDate') ){
                              $date = $this->request->session()->read('Report.TotaleTasks.startDate');
		                ...
		                <div class="col-lg-6 col-md-12">
                          ...
                          <?php
                            $date = '';
                            if($this->request->session()->read('Report.TotaleTasks.endDate') ){
                              $date = $this->request->session()->read('Report.TotaleTasks.endDate');
                        ...
                        <div class="form-group">
                          ...
                          <select class="form-control select2" id="yearID" style="width: 100%;">
                            <?php foreach ($years as $year) {
                              if($this->request->session()->read('Report.TotaleTasks.year') && $this->request->session()->read('Report.TotaleTasks.year')== $year){
                    ...
                    <!--  <div class="col-md-12">
                        <div class="form-group">
                          <?php  $checked = "";
                          if($this->request->session()->read('Report.TotaleTasks.year') && $this->request->session()->read('Report.TotaleTasks.year')== -1){
                              $checked = "checked";
                          }
                          ?>
                          <label class="checkbox-inline"><input id="escludiAnno" type="checkbox" value="" <?php echo $checked; ?>>Escludi Anno </label>
                        </div>
                      </div> -->
                      <div class="col-md-12">
                        <div class="form-group">
                          ...
                          <select multiple class="form-control select2" id="officeID" style="width: 100%;">
                            <?php
                              ...
                              if($this->request->session()->read('Report.TotaleTasks.office'))
                                $selected_office = explode(",",$this->request->session()->read('Report.TotaleTasks.office'));
                              else
      ...
      <div class="box">
              ...
              <div class="box-body box-table">
                <table id="table-report" class="table table-bordered table-striped table-hover">
                  <thead>
                    <tr>
                      <th data-toggle="tooltip" data-html="true" title="Nome cliente">Cliente</th>
                      <th data-toggle="tooltip" data-html="true" title="Causale lavoro">Causale</th>
                      <th data-toggle="tooltip" data-html="true" title="Nome operatore">Operatore</th>
                      <th data-toggle="tooltip" data-html="true" title="Eventuali note">Note</th>
                      <th data-toggle="tooltip" data-html="true" title="Ora e data inizio attività">Inizio</th>
                      <th data-toggle="tooltip" data-html="true" title="Ora e data fine attività">Fine</th>
                      <th data-toggle="tooltip" data-html="true" title="Durata attività in ore e minuti">Durata</th>
                      <th>Completato</th>
                    </tr>
                  </thead>
                  <tbody>
