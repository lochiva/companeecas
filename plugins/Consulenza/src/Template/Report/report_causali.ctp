  ...
  $("#table-report").trigger("update");
    ...
    function setTable(){
        ...
        if($("#datepicker").val() != '' && $("#datepicker2").val() != '' && $("#datepicker").val().length == 10 && $("#datepicker2").val().length == 10 ){
          ...
          $('#reportTitle').html('<i  class="fa fa-th"></i> REPORT SINTETICO CON CAUSALE  DAL ' + startDate + ' AL '+ endDate +' '+ lista_studi + ' RIF '+$('#yearID option:selected').text());

        }else{

          $('#reportTitle').html('<i  class="fa fa-th"></i> REPORT SINTETICO CON CAUSALE ' + $('#yearID option:selected').text() + ' ' + lista_studi);
        }
        ...
        $("#table-report").tablesorter({
              ...
              headers: { 2: { filter: false},3: { filter: false },4: { filter: false } }
                ...
                container: $(".pager"),
                ajaxUrl : pathServer + 'consulenza/Ws/getReportScostamentiCausale/'+yearID+'/'+officeID+'/?{filterList:filter}&{sortList:column}&size={size}&page={page}&startDate='+startDate+'&endDate='+endDate+' ',
    ...
    $(document).ready(function(){
      ...
      $('#xls_export').click(function(){
          window.open(pathServer + 'consulenza/report/report_causali/'+$('#yearID').val()+'/'+($('#officeID').val() != null ? $('#officeID').val() : -1)+'/xls','_self');
      });
...
<section class="content-header">
  <h1>
    Report sintetico con causale
    <small>Report sintetico per causale con cliente, ore pianificate, programate e consuntivate</small>
  </h1>
  <input type="hidden" id="clienteID" value="" />
  <ol class="breadcrumb">
    <li><i class="fa fa-pie-chart"></i> Report sintetico</li>
    <li class="active">Report con causale</li>
  </ol>
</section>
                ...
                <div class="box-body">
                	<div class="row">
                		<div class="col-lg-6 col-md-12">
                          ...
                          <?php
                            $date = '';
                            if($this->request->session()->read('Report.ScostamentiCausale.startDate') ){
                              $date = $this->request->session()->read('Report.ScostamentiCausale.startDate');
		                ...
		                <div class="col-lg-6 col-md-12">
                          ...
                          <?php
                            $date = '';
                            if($this->request->session()->read('Report.ScostamentiCausale.endDate') ){
                              $date = $this->request->session()->read('Report.ScostamentiCausale.endDate');
                        ...
                        <div class="form-group">
                          ...
                          <select class="form-control select2" id="yearID" style="width: 100%;">
                            <?php foreach ($years as $year) {
                              if($this->request->session()->read('Report.ScostamentiCausale.year') && $this->request->session()->read('Report.ScostamentiCausale.year')== $year){
                        ...
                        <div class="form-group">
                          ...
                          <select multiple class="form-control select2" id="officeID" style="width: 100%;">
                            <?php
                              ...
                              if($this->request->session()->read('Report.ScostamentiCausale.office'))
                                $selected_office = explode(",",$this->request->session()->read('Report.ScostamentiCausale.office'));
                              else
      ...
      <div class="box">
              ...
              <div class="box-body box-table">
                <table id="table-report" class="table table-bordered table-striped table-hover">
                  <thead>
                    <tr>
                      <th data-toggle="tooltip" data-html="true" title="Nome del cliente">Cliente</th>
                      <th data-toggle="tooltip" data-html="true" title="Causale del lavoro">Causale</th>
