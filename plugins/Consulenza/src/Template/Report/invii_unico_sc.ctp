...
<?php if($user['role']!='admin' && $user['level']<'100' ){ // check admin ?>
<style type="text/css">
.invia_btn,.invia_btn_irap{
  display:none;
}
</style>
<?php } ?>
...
<script>
    ...
    <?php
      ...
      if($user['role']=='admin' && $user['level']>='100' ){ // check admin

    ?>

       if(confirm('Inviare UNICO per questo cliente?')){
          ...
          <?php } //fine check admin ?>

    }

    function invia_report_irap(id){
    ...
    ?>
       ...
       if(confirm('Inviare IRAP per questo cliente?')){

          sendReportIrap(id,function(id,data){
              $('#span_sent_irap-'+id).html('<div class="text-green text-center"><i class="fa fa-check-circle"></i> INVIATO</div>');
          });
    ...
    function invia_report_bilancio(id)
    {
       <?php
        ...
        ?>
           ...
           if(confirm('Depositare il bilancio per questo cliente?')){

              sendReport(id,function(id,data){
                  $('#span_sent-'+id).html('<div class="text-green text-center"><i class="fa fa-check-circle"></i> DEPOSITATO</div>');
                  $('#span_milestone-'+id).html(data.data);
              });
    ...
    function setTable(){
        ...
        $("#table-report").tablesorter({
              ...
              headers: { 9: { filter: false, sorter: false},10: { filter: false, sorter: false} ,11: { filter: false, sorter: false} }
          }).tablesorterPager({
                ...
                container: $(".pager"),
                ajaxUrl : pathServer + 'consulenza/Ws/getInviiCausaliUNICOSC/'+causaleID+'/'+yearID+'/'+officeID+'?{filterList:filter}&{sortList:column}&size={size}&page={page}',
    ...
    $('#officeID').change(function(){
        ...
        $(document).on("click", '#inviaIrapBtn', function(event) {
            invia_report_irap($(this).attr('jobs-order-id'));
        });

        $(document).on("click", '#inviaBtnBilancio', function(event) {
            invia_report_bilancio($(this).attr('jobs-order-id'));
        });

        $('#xls_export').click(function(){
            window.open(pathServer + 'consulenza/report/invii_unico_sc/' + $('#causaleID').val()+'/'+$('#yearID').val()+'/'+($('#officeID').val() != null ? $('#officeID').val() : -1)+'/xls','_self');
        });
...
<section class="content-header">
  <h1>
    Report UNICO/IRAP SC
    <small></small>
  </h1>
  <ol class="breadcrumb">
    <li><a><i class="fa fa-line-chart"></i> Report invii</a></li>
    <li class="active">Report UNICO/IRAP</li>
  </ol>
</section>
                     ...
                     <div class="form-group">
                        ...
                        <select class="form-control select2" id="causaleID" style="width: 100%;">
                          <?php foreach ($jobs as $job) {
                            if($this->request->session()->read('Report.InviiCausaliUnicoSC.causaleId') && $this->request->session()->read('Report.InviiCausaliUnicoSC.causaleId')== $job['id']){
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" id="yearID" style="width: 100%;">
                          <?php foreach ($years as $year) {
                            if($this->request->session()->read('Report.InviiCausaliUnicoSC.year') && $this->request->session()->read('Report.InviiCausaliUnicoSC.year')== $year){
                    ...
                    <div class="form-group">
                        ...
                        <select multiple class="form-control select2" id="officeID" style="width: 100%;">
                          <?php
                            ...
                            if($this->request->session()->read('Report.InviiCausaliUnicoSC.office'))
                              $selected_office = explode(",",$this->request->session()->read('Report.InviiCausaliUnicoSC.office'));
                            else
                  ...
                  <table id="table-report" class="table table-bordered table-striped table-hover">
                    <thead>
                      <tr>
                        ...
                        <th>Cons. Bilancino</th>
                        <th>P. IVA</th>
                        <th>Operatore Contabilit&agrave;</th>
                        <th>Operatore UNICO</th>
                        <th>Stato Bilancio</th>
                        <th>Stato contabilità</th>
                        <th>Stato UNICO</th>
                        <th>UNICO inviato</th>
                        <th>IRAP inviato</th>
                        <th>Bilancio Depositato</th>
                        <th>Note</th>
                      </tr>
                    </thead>
                    <tbody>
