...
<script>
    ...
    function setTable(){
        ...
        $("#table-report").tablesorter({
              ...
              headers: { 9: { filter: false, sorter: false},10: { filter: false, sorter: false} }
          }).tablesorterPager({
                ...
                container: $(".pager"),
                ajaxUrl : pathServer + 'consulenza/Ws/getInviiCausaliUNICOENC/'+causaleID+'/'+yearID+'/'+officeID+'?{filterList:filter}&{sortList:column}&size={size}&page={page}',
    ...
    $('#officeID').change(function(){
        ...
        $('#xls_export').click(function(){
            window.open(pathServer + 'consulenza/report/invii_unico_enc/' + $('#causaleID').val()+'/'+$('#yearID').val()+'/'+($('#officeID').val() != null ? $('#officeID').val() : -1)+'/xls','_self');
        });
                     ...
                     <div class="form-group">
                        ...
                        <select class="form-control select2" id="causaleID" style="width: 100%;">
                          <?php foreach ($jobs as $job) {
                            if($this->request->session()->read('Report.InviiCausaliUnicoENC.causaleId') && $this->request->session()->read('Report.InviiCausaliUnicoENC.causaleId')== $job['id']){
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" id="yearID" style="width: 100%;">
                          <?php foreach ($years as $year) {
                            if($this->request->session()->read('Report.InviiCausaliUnicoENC.year') && $this->request->session()->read('Report.InviiCausaliUnicoENC.year')== $year){
                    ...
                    <div class="form-group">
                        ...
                        <select multiple class="form-control select2" id="officeID" style="width: 100%;">
                          <?php
                            ...
                            if($this->request->session()->read('Report.InviiCausaliUnicoENC.office'))
                              $selected_office = explode(",",$this->request->session()->read('Report.InviiCausaliUnicoENC.office'));
                            else
