...
<script>
    ...
    function setTable(){
        ...
        $("#table-report").tablesorter({
              ...
              headers: { 8: { filter: false, sorter: false},9: { filter: false, sorter: false} }
                ...
                container: $(".pager"),
                ajaxUrl : pathServer + 'consulenza/Ws/getInviiCausaliUNICO/'+causaleID+'/'+yearID+'/'+officeID+'?{filterList:filter}&{sortList:column}&size={size}&page={page}',
    ...
    $('#officeID').change(function(){
        ...
        $('#xls_export').click(function(){
            window.open(pathServer + 'consulenza/report/invii_causali_unico/' + $('#causaleID').val()+'/'+$('#yearID').val()+'/'+($('#officeID').val() != null ? $('#officeID').val() : -1)+'/xls','_self');
        });
                     ...
                     <div class="form-group">
                        ...
                        <select class="form-control select2" id="causaleID" style="width: 100%;">
                          <?php foreach ($jobs as $job) {
                            if($this->request->session()->read('Report.InviiCausaliUnico.causaleId') && $this->request->session()->read('Report.InviiCausaliUnico.causaleId')== $job['id']){
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" id="yearID" style="width: 100%;">
                          <?php foreach ($years as $year) {
                            if($this->request->session()->read('Report.InviiCausaliUnico.year') && $this->request->session()->read('Report.InviiCausaliUnico.year')== $year){
                    ...
                    <div class="form-group">
                        ...
                        <select multiple class="form-control select2" id="officeID" style="width: 100%;">
                          <?php
                            ...
                            if($this->request->session()->read('Report.InviiCausaliUnico.office'))
                              $selected_office = explode(",",$this->request->session()->read('Report.InviiCausaliUnico.office'));
                            else
