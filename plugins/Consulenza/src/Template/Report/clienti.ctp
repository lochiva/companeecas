  ...
  $("#table-report").trigger("update");
    ...
    function setTable(){
        ...
        if($("#datepicker").val() != '' && $("#datepicker2").val() != '' && $("#datepicker").val().length == 10 && $("#datepicker2").val().length == 10 ){
          ...
          $('#reportTitle').html('<i  class="fa fa-th"></i> REPORT SINTETICO CLIENTI DAL ' + startDate + ' AL '+ endDate +' '+ lista_studi + ' RIF '+$('#yearID option:selected').text());

        }else{

          $('#reportTitle').html('<i  class="fa fa-th"></i> REPORT SINTETICO CLIENTI ' + $('#yearID option:selected').text() + ' ' + lista_studi);
        }
        ...
        $("#table-report").tablesorter({
              ...
              headers: { 1: { filter: false},2: { filter: false },3: { filter: false } }
                ...
                container: $(".pager"),
                ajaxUrl : pathServer + 'consulenza/Ws/getReportScostamentiClienti/'+yearID+'/'+officeID+'/?{filterList:filter}&{sortList:column}&size={size}&page={page}&startDate='+startDate+'&endDate='+endDate+' ',
    ...
    $(document).ready(function(){
      ...
      $('#xls_export').click(function(){
          window.open(pathServer + 'consulenza/report/clienti/'+$('#yearID').val()+'/'+($('#officeID').val() != null ? $('#officeID').val() : -1)+'/1','_self');
      });
...
<section class="content-header">
  <h1>
    Report sintetico per clienti
    <small>Report delle ore pianificate, programate e consuntivate  per ogni  cliente </small>
  </h1>
  <ol class="breadcrumb">
    <li><a><i class="fa fa-pie-chart"></i> Report sintetico</a></li>
    <li class="active">Report per clienti</li>
  </ol>
</section>
