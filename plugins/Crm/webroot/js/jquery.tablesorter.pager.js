...
;(function($) {
	...
	$.extend({ tablesorterPager: new function() {
		...
		updatePageDisplay = function(table, p, completed) {
			...
			/*console.log('pagina: ' + parseInt(p.page+1));
			console.log('size: ' + parseInt(p.size));
			console.log('filteredRows: ' + parseInt(p.filteredRows));
			console.log('filteredPages: ' + parseInt(p.filteredPages));*/
			
			var totale_pagine_temp = Math.ceil(parseInt(p.filteredRows) / parseInt(p.size));
			var html_pagine_gen = "";
			
			/*for (var i = 1; i <= totale_pagine_temp; i++)
			{
				html_pagine_gen+="<li class=\"removable_pages\"><a href=\"javascript:void(0)\">" + i + "</a></li>";
			}
			*/
			
			//console.log(html_pagine_gen);
			
			$('.removable_pages').remove();
			
			$(html_pagine_gen).insertAfter('.pagination > li:first');
			
			$('#num-pagine').text('Pagina ' + parseInt(p.page+1) + ' di ' + totale_pagine_temp + ' di ' + parseInt(p.filteredRows) + ' risultati');
		},
