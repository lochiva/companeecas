/**
 * Variabile usata dalla modale 'person' per capire se deve fare il reload della tabella.
 * @type {Boolean}
 */
window.inPeoplePage = true;
$(document).ready(function(){
	...
	$("#table-people").tablesorter({
        ...
        widgetOptions: {
          ...
          filter_functions:{
              // Variabile servicesListFilter dichiarata in 'People/index.twig' dal php
              7 : servicesListFilter
          }
        },
        headers: { }
    }).tablesorterPager({
          ...
          ajaxUrl : pathServer + 'progest/ws/people/table?{filterList:filter}&{sortList:column}&size={size}&page={page}',
...
$(document).on('click','.delete',function(e){
	...
	if(confirm('Si è sicuri di voler cancellare la persona?')){
		deletePerson($(this).attr('data-id'));
	}
...
$(document).on('click','.print-table',function(e){
		var url = $("#table-people")[0].config.pager.last.ajaxUrl;
		url = url.replace('table','indirizzario');
		window.open(url, '_blank');
});

//########################################################################################################################
//Gestione Download XLS tabella
$(document).on('click','.excel-table',function(e){
		 var url = $("#table-people")[0].config.pager.last.ajaxUrl;
		 url = url.replace('table','indirizzario/xls');
...
function deletePerson(id){
	...
	$.ajax({
	    url : pathServer + "progest/ws/people/delete/" + id,
	    type: "GET",
	    dataType: "json",
...
function reloadTablePeople(){
	...
	var pager = $("#table-people")[0].config.pager;
	setTableSorterTempPager('table-people', [(pager.page+1),pager.size] );
	$("#table-people").trigger("update");
    lastSortList=$("#table-people")[0].config.sortList;
    $("#table-people").trigger("sorton", false);
    $("#table-people").trigger("sorton", [lastSortList]);
