...
$(document).ready(function(){
  ...
  $('#calendar').fullCalendar({
    ...
    events: pathServer + 'calendar/ws/getCalendarEvents/' +$('#user-caledar-view').val(),
    locale: 'it',
    editable: true,
    ...
    eventRender: function(event, element) {
        //console.log(event);console.log(idTagScadenzario);
        if(event.azienda_denominazione !== null){
            $(element).children('.fc-content').append(' <b>Azienda:</b> '+event.azienda_denominazione);
        }
        if(event.nome_order !== undefined && event.nome_order !== null ){
            $(element).children('.fc-content').append(' </br><b>Ordine:</b> '+event.nome_order);
        }
        if(event.tags !== undefined && event.tags.length !== 0){
          var found = false;
          for(var i=0; i<event.tags.length && !found ;i++){
            if(event.tags[i].id == idTagScadenzario){
              $(element).children('.fc-content').prepend(' <b>Scadenzario</b> ');
              found = true;
            }
          }
        }
        /*if(event.id_google != undefined && event.id_google != ""){
            $(element).children('.fc-content').prepend('<i class="fa fa-google" aria-hidden="true" style="float:right;"></i> ');
        }*/
  ...
  $('#myModalCalendar').on('hidden.bs.modal', function () {
...
function myDeleteEvent(event){
  ...
  //alert(idEvent);
  deleted = $.ajax({
              url : pathServer + "calendar/ws/deleteEvent/" + idEvent,
              type  : "post",
...
function mySaveEvent(event,delta){
  ...
  saved = $.ajax({
              ...
              data : {id:id,title:title,allDay:allDay,start:start,end:end,note:note,repeated:repeated,id_google:id_google,
                  id_user:id_user, backgroundColor:backgroundColor},
              dataType : "json",
              async: false,
              success : function (data,stato) {
...
function clearModale(){
  ...
  $('#repeatedEndType').change();$('#inputColor').colorselector("setValue", '#3a87ad');
  ...
  $('#myModalCalendar #eventDetailsNote').html('');
...
function fillModaleCalendar(event){
  ...
  if(event.id_timetask != 0){
    $('#taskNumber').val(event.number_timetask);
    $('#taskClient').val(event.client_timetask);
    $('#taskProject').val(event.project_timetask);
    $('#idTask').val(event.id_timetask);
    $('#timetask-data').show();
    $('#idTimeTimetask').val(event.id_time_timetask);
  }
  ...
  if(event.id_azienda !== 0){
    $("#idAzienda").append('<option value="'+event.id_azienda+'">'+event.azienda_denominazione+'</option>');
    fillingModal = true;
    $('#idAzienda').val(event.id_azienda).trigger("change");
    loadContattiAzienda(event.id_azienda,event.id_contatto);
    loadOrdersAzienda(event.id_azienda,event.id_order);
    fillingModal = false;
  }
...
function loadTempCalendar()
{
  ...
  if(temp[userId] == undefined || temp[userId] == null){
    return toRet;
  }
  toRet.view = temp[userId].view;
  toRet.date = new Date(temp[userId].date);
  toRet.default = false;
