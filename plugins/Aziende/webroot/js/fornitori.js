...
$(document).on('click','.delete-passive-invoice',function(e){

	e.preventDefault();

	if(confirm('Si è sicuri di voler eliminare la fattura?')){
		deleteFatturaPassiva($(this).attr('data-id'));
	}
...
$(document).on('click','.edit-passive-invoice',function(e){

	disableInputModale();

	var idFattura = $(this).attr('data-id');
	//alert('edit ' + idAzienda);
	loadInputModalePassiva(idFattura);
...
$(document).on('click','.send-invoice',function(e){
	...
	$.ajax({
	    url : pathServer + "aziende/Ws/sendInvoice/" + idFattura,
	    type: "POST",
	    dataType: "json",
...
function deleteFatturaPassiva(id){
...
function loadInputModalePassiva(idFattura){
	...
	$.ajax({
	    ...
	    success : function (data,stato) {
	        ...
	        if(data.response == "OK"){
						$("#myFormFatturaPassiva #idFornitore").append('<option value="'+data.data.id_issuer+'">'+data.data.issuer.denominazione+'</option>');
						if(idFornitore == 'all'){
							$('#myFormFatturaPassiva #idFornitore').val(data.data.id_issuer).trigger("change");
						}
						if(data.data.order != null){
								$("#myFormFatturaPassiva #idOrder").append('<option value="'+data.data.id_order+'">'+data.data.order.name+'</option>');
								$('#myFormFatturaPassiva #idOrder').val(data.data.id_order).trigger("change");
						}
						...
						if(data.data.xml != undefined && data.data.xml != ""){
							$('.xml').html('<a target="_blank" href="'+pathServer+'aziende/fornitori/getAttachment/'+data.data.xml+'">FILE XML</a>');
						}
...
function reloadTableFatturePassive(){
...
function clearModale(){
	if(idFornitore == 'all'){
		$("#myFormFatturaPassiva #idFornitore").html("");
	}
	$("#myFormFatturaPassiva #idOrder").html("");
	$('#myFormFatturaPassiva #idFornitore').val('0').trigger("change");
	$('#myFormFatturaPassiva #idOrder').val('0').trigger("change");
